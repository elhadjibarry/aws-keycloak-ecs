...
### 1.2 Push the image to Amazon ECR
```bash
...
The [keycloak-vpc-rds.yaml](cloudformation/keycloak-vpc-rds.yaml) template creates:
- ECS tasks execution role
- A VPC with public and private subnets
- An Internet Gateway and NAT Gateways
- Security groups for the ALB, RDS and ECS tasks
...
| `KeycloakAdminUsername` | The admin username for Keycloak (Password will be generated by Secrets Manager)                                            | String | `admin`        |
| `DomainName` | The domain name that will be used to create a certificate in ACM for the ALB                                            | String |        |
| `HostedZoneId` | The ID of the Route 53 Hosted Zone of the domain to create a certificate in ACM                                            | String |       |

### 2.2 Resources
The template creates the following resources:

- **ECS Task Execution Role**: IAM role that allows ECS tasks to make API requests to AWS services.
...
- **Public Subnet 1**: Public subnet in the first Availability Zone for the the Application Load Balancer (ALB).
...
- **RDS Private Subnet 1**: Private subnet for RDS in the first Availability Zone.
- **RDS Private Subnet 2**: Private subnet for RDS in the second Availability Zone for high availability and fault tolerance.
...
- **DB Subnet Group**:Subnet group for the RDS instance. The RDS instance will be created in the private subnets in the two Availability Zones (Multi AZ).
...
- **Certificate**: ACM Public certificate for the specified domain name using DNS validation. This certificate will be used by the Application Load Balancer to enable HTTPS traffic.
...
- An Application Auto Scaling group for the Keycloak service
...
| `PublicSubnetIds`       | The list of Public Subnet IDs for the Load Balancer   | `List<AWS::EC2::Subnet::Id>` |
| `ECSPrivateSubnetIds`   | The list of Private Subnet IDs for the ECS tasks      | `List<AWS::EC2::Subnet::Id>` |
...
| `DomainName`            | The domain name for the alias record                  | `String`                     |
| `RDSInstanceEndpoint`            | he RDS instance endpoint for Keycloak database                 | `String`                     |
...
- **Keycloak Scalable Target**: Configures auto-scaling for the Keycloak service based on CPU utilization. The minimum capacity is 1 task and the maximum capacity is 5 tasks.
- **Keycloak Load Balancer**: Application Load Balancer (ALB) to distribute traffic to the Keycloak service.
...
- **Route 53 Alias Record**: Route 53 alias record to point to the ALB, enabling access to the Keycloak application via a custom domain with HTTPS.
